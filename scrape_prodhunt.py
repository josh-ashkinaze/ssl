...
# parser = argparse.ArgumentParser(description='Fetch ProductHunt posts within a given timeframe.')
# parser.add_argument('--start_date', type=str, help='Start date in<\ctrl3348>-MM-DD format', required=True)
# parser.add_argument('--end_date', type=str, help='End date in<\ctrl3348>-MM-DD format', required=True)
# args = parser.parse_args()

# Hardcode start and end dates
...
def fetch_posts_in_timeframe(start_date_str, end_date_str, token, first=20, after=None):
    ...
    if response.status_code == 429:
        ...
        print(f"Rate limited. Suggested retry after: {retry_after} seconds")
    ...
    if response.status_code == 200:
        ...
        if "errors" in data:
            ...
            print(f"Full error: {data['errors']}")
...
def fetch_all_posts_in_timeframe(start_date_str, end_date_str, token, max_pages=20):
    ...
    while has_next_page and page <= max_pages:
        print(f"Fetching page {page} for timeframe {start_date_str} to {end_date_str}...")
        ...
        try:
            ...
            if "posts" in result:
                ...
                print(f"Retrieved {len(result['posts'])} posts (total: {len(all_posts)} of {total_count})")
            ...
            print(f"Rate limit encountered. Waiting for {wait_seconds} seconds before retrying page {page}...")
            time.sleep(wait_seconds)
            print(f"Resuming fetch for page {page}...")
    ...
    while current_date <= end_date:
        ...
        try:
            ...
            if isinstance(result, str):  # Error occurred
                ...
                all_posts.extend(result["posts"])
                total_count += result["total_count"]
                print(f"Added {result['count']} posts from {current_date_str}")

        except RateLimitException as e:
            retry_after = str(e).split("Retry after ")[-1].split(" ")[0]
            wait_seconds = int(retry_after)
            print(f"Rate limit encountered while processing {current_date_str}. Waiting for {wait_seconds} seconds...")
            time.sleep(wait_seconds)
            print(f"Resuming fetch for {current_date_str}...")
...
def save_posts_to_json(posts_data, output_file):
    ...
    print(f"Saved {posts_data['count']} posts to {output_file}")
...
if __name__ == "__main__":
    developer_token = PRODUCT_HUNT_API_KEY
    ...
    if isinstance(result, str):
        ...
        print(f"Retrieved {result['count']} posts of {result['total_count']} total")


        save_posts_to_json(result, f"producthunt_posts_{start_date_str}_to_{end_date_str}.json")
        save_posts_to_csv(result, f"producthunt_posts_{start_date_str}_to_{end_date_str}.csv")