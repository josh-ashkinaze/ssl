#!/usr/bin/env python3
"""
Generate RoT (rule-of-thumb) persuasive arguments for SSL.

Inputs
- data/clean/human_stimuli_stratified.csv
  expected cols: rot, action, situation|context, ssl_domain, agreement_condition

Outputs
- data/clean/ai_rot_stimuli.csv
...
def main():
    ...
    # Map optional context from input for completeness in output
    if "context" in df.columns:
        context_series = df["context"]
    elif "situation" in df.columns:
        context_series = df["situation"]
    else:
        context_series = pd.Series([""] * len(df))
    ...
    rows = []
    with tqdm(total=len(df), desc="Generating RoT") as pbar:
        for i, row in df.iterrows():
            ...
            rows.append({
                ...
                "rating": rating,
                "llm_response_rot": llm_text,
                "llm_response_action_context": "",
